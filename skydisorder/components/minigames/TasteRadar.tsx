...
export default function TasteRadar({ onComplete }: Props) {
  ...
  return (
    <div className="pixel-panel" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px', gap: '20px' }}>
      ...
      {result === 'success' && <div style={{ color: 'var(--neon-green)', fontSize: '24px', marginTop: '10px' }}>SEQUENCE CORRECT!</div>}
      {result === 'fail' && <div style={{ color: 'var(--neon-red)', fontSize: '24px', marginTop: '10px' }}>WRONG SEQUENCE!</div>}
    </div>
  );
}
